...
class IsBookmarkedUseCase @Inject constructor(private val bookmarksRepository: BookmarksRepository) {

    suspend operator fun invoke(repository: Repository): Boolean {
        return bookmarksRepository.isRepositoryBookmarked(repository)
    }
}